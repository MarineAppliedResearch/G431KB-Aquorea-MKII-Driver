...
static void wizchip_spi_writebyte(uint8_t byte)
{
    ...
    (void)HAL_SPI_TransmitReceive(&hspi1, &byte, &rx, 1, 10);
}
...
static void wizchip_spi_readburst(uint8_t* pBuf, uint16_t len)
{
    ...
    for (uint16_t i = 0; i < len; i++)
        HAL_SPI_TransmitReceive(&hspi1, &tx, &pBuf[i], 1, HAL_MAX_DELAY);
}
...
static void wizchip_spi_writeburst(uint8_t* pBuf, uint16_t len)
{
    uint8_t rx;
    for (uint16_t i = 0; i < len; i++)
        HAL_SPI_TransmitReceive(&hspi1, &pBuf[i], &rx, 1, HAL_MAX_DELAY);
}
